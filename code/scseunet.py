...
from senet import se_resnext50_32x4d, senet154,  SCSEModule
...
class EncoderDecoder(AbstractModel):
    def __init__(self, num_classes, num_channels=3, encoder_name='resnet34', isResidual=False, isJPEG=False,resRatio=0.02,qf=92):
        ...
        encoder = encoder_params[encoder_name]['init_op'](pretrained=False, num_channels=num_channels)
        ...
        # encoder2 = encoder_params[encoder_name]['init_op'](pretrained=False)
        ...
        if self.isJPEG:
            #self.diff_jpeg = DiffJPEG(height=256, width=256, differentiable=True)
            self.diff_jpeg = DiffJPEG(differentiable=True)

    # noinspection PyCallingNonCallable
    def forward(self, x):
        if len(x) == 3:
            x,qf,in_isjpg = x[0],x[1],x[2]
        x_ori = x 
        _, _, h0, w0 = x.shape
        enc_results = []
        ...
        for idx, bottleneck in enumerate(self.bottlenecks):
            ...
            x = bottleneck(x, enc_results[rev_idx - 1])#, enc_results[rev_idx - 1])
        ...
        if self.isResidual:
            ...
            out = self.resRatio * f + (1-self.resRatio) * x_ori

        else:
            out = f
        #out = self.dropout(out)
        if self.isJPEG:
            out = self.diff_jpeg([(out+1)/2,self.qf])
            out = (out - 0.5) * 2
    ...
    def make_final_classifier(self, in_filters, num_classes):
        ...
        return nn.Sequential(nn.Conv2d(in_filters, num_classes, 1, padding=0), nn.Tanh())
